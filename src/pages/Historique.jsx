...
import { getHistorique, getHistoriqueRecherche } from "../services/HistoriqueApi";
import { FaChevronLeft, FaChevronRight, FaColumns, FaHistory, FaSearch, FaStepBackward, FaStepForward, FaTimes, FaFilter } from "react-icons/fa";
...
const Historique = () => {
  ...
  useEffect(() => {
    document.title = "Historiques"
    const getData = async () => {
      try {
        const d = await getHistorique();
        setData(d);
        setIsLoading(false);
      } catch (error) {
  ...
  const recherche = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await getHistoriqueRecherche(filters);
      setData(result);
    } catch (error) {
      console.error('Error refreshing data:', error);
    } finally {
    ...
    <div className="flex flex-col h-screen">
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold text-gray-800">Historique des Tests</h2>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={(e) => setFilters({ ...filters, typeProduit: e.target.value })}
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-gray-700">Groupe de produit</label>
              <input
                className="p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                type="text"
                placeholder="Rechercher par groupe de produit"
                value={filters.groupeProduit}
                onChange={(e) => setFilters({ ...filters, groupeProduit: e.target.value })}
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              >
                <option value="">Tous les activités</option>
                <option value="DEROULEMENT">Déroulement</option>
            ...
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-600"></div>
              ...
              <table className="min-w-full table-auto">
                <thead className="bg-blue-600 text-white">
                  <tr>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Nom d'utilisateur</th>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Code contrat</th>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Date de test</th>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Type Produit</th>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Groupe Produit</th>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Résultat du Test</th>
                    <th className="text-left px-6 py-3 text-sm font-medium uppercase tracking-wider">Activité</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {currentData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4 text-sm font-medium">{row.codeContrat}</td>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{row.produit?.nom}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{row.grProduit?.nom}</td>
                      <td className="px-6 py-4 text-sm">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium
                          ${row.statutTest === "TestNonConcluant" ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}`}>
                          {row.statutTest === "TestNonConcluant" ? "Non Concluant" : "Concluant"}
                        </span>
                      </td>
